...
1. *Temel Aritmetik*
2. *Mantık Kapıları*
3. Bit Kaydırma/Karşılaştırma
---
## 1. Temel Aritmetik
MARS, Mips assemblyde registerlar'da değerlerde aritmetik işlemler şöyledir
### Add
- iki registerı toplamak için kullanılan bir instructiondır.
...
```
### Sub
İki registerı çıkarmak için kullanılan bir instructiondır.
```asm
.text
main:
...
```
### Addi
- Registera bir sabit sayı değeri eklemek için kullanılan bir instructiondır.
```asm
.text
main:
...
```
### Mull ve Div
- bu işlemlere bu kursumda bakmak istemiyorum özellikle div **HI** ve **LO** içerdiği için Docs'taki yardımcı kaynaklardan bakmak daha iyi olacaktır.
...
### Karşılaştırma: SLT (Set on Less Than)
